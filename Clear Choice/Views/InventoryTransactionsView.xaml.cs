    ...
    public partial class InventoryTransactionsView : UserControl, ISTabView
    {
        ...
        private void LoadTransactions(int type)
        {
            try
            {
                ...
                if (type == 0)
                {
                    data = db.Select("*", "view_transactions_sites");
                }
                else if (type == 1)
                {
                    data = db.Select("*", "view_transactions_clients");
                }
                else
                {
                    data = db.Select("*", InventoryTransaction.Table, InventoryTransaction.Fields.ClientType.ToString() + " = " + type);
                }
                ...
                gridViewData = data;
                dgTransactions_AutoGeneratedColumns(null, new EventArgs());
            }
            catch (Exception ex)
            {
        ...
        private void dgTransactions_AutoGeneratedColumns(object sender, EventArgs e)
        {
            ...
            foreach (DataGridColumn column in dgTransactions.Columns)
            {
                ...
                if (headerText.Equals("Recieved By") && cmboType.SelectedIndex > 1)
                {
                    dgTransactions.Columns[index].Visibility = System.Windows.Visibility.Collapsed;
                }
                else if (textMap.ContainsKey(headerText))
                {
